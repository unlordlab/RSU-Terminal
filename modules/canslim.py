...
def create_market_dashboard(market_data: dict) -> go.Figure:
    ...
    fig = make_subplots(rows=2, cols=2,
                        subplot_titles=('S&P 500', 'NASDAQ 100', 'Russell 2000', 'VIX'),
                        vertical_spacing=0.15, horizontal_spacing=0.1)
    pairs = [('SPY', COLORS['primary'], 1, 1), ('QQQ', '#2962FF', 1, 2),
             ('IWM', '#00BCD4', 2, 1), ('^VIX', COLORS['danger'], 2, 2)]
    for ticker, color, row, col in pairs:
        if ticker in market_data['data']:
            df = market_data['data'][ticker]['data']
    ...
    fig.update_layout(
        ...
        title=dict(text=f"Market Score: {market_data['score']}/100 — {market_data['phase']}",
                   font=dict(size=15, color=market_data['color']))
    )
    fig.update_xaxes(showgrid=True, gridcolor=COLORS['bg_card'], color='white')
    fig.update_yaxes(showgrid=True, gridcolor=COLORS['bg_card'], color='white')
    return fig
...
def render():
    ...
    with tab4:
        ...
        st.markdown('<h2>🤖 MACHINE LEARNING PARA CAN SLIM</h2>', unsafe_allow_html=True)

        if not SKLEARN_AVAILABLE:
            st.markdown(f"""
            <div class="risk-box">
                <div style="font-family:'VT323',monospace;color:{COLORS['warning']};font-size:1.2rem;">
                    ⚠️ MÓDULO NO DISPONIBLE
                </div>
                <p>scikit-learn no está instalado:</p>
                <code style="color:{COLORS['primary']};">pip install scikit-learn joblib</code>
            </div>""", unsafe_allow_html=True)

        col1, col2 = st.columns(2)
        with col1:
            st.markdown('<h3>ENTRENAMIENTO DEL MODELO</h3>', unsafe_allow_html=True)
            st.markdown(f"""
            <div class="terminal-box">
                <p>El modelo ML analiza patrones históricos CAN SLIM:</p>
                <ul>
                    <li>Gradient Boosting Classifier (sklearn)</li>
                    <li>Features: crecimiento, momentum, volumen, institucional</li>
                    <li>Target: outperformance vs S&P 500 a 3 meses</li>
                    <li>Modelo guardado en disco para reutilización</li>
                </ul>
            </div>""", unsafe_allow_html=True)
            if st.button("🚀 ENTRENAR MODELO", type="primary", disabled=not SKLEARN_AVAILABLE):
                st.info("Entrena el modelo con datos históricos de candidatos CAN SLIM pasados.")
                    ...
                    if res:
                        ...
                        <div class="terminal-box" style="text-align:center;">
                            <div style="font-family:'VT323',monospace;color:#888;font-size:1rem;letter-spacing:2px;">
                                PROBABILIDAD DE OUTPERFORMANCE
                            </div>
                            <div style="font-family:'VT323',monospace;color:{color};font-size:5rem;
                            margin:10px 0;text-shadow:0 0 20px {color}66;">{prob:.1%}</div>
                            <div style="font-family:'Courier New',monospace;color:#666;font-size:.85rem;">
                                Basado en características CAN SLIM históricas
                            </div>
                        </div>""", unsafe_allow_html=True)
                    else:
    ...
    with tab5:
        ...
        st.markdown('<h2>📈 BACKTESTING CON ZIPLINE</h2>', unsafe_allow_html=True)
        ...
        if st.button("▶️ EJECUTAR BACKTEST", type="primary", disabled=not ZIPLINE_AVAILABLE):
            with st.spinner("Ejecutando simulación histórica..."):
                st.success("Backtest completado [demo]")
