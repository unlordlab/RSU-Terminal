...
# ============================================================
# JOB NOCTURNO — CARGA DE CACHE JSON (v4.0.1 FIX)
# ============================================================

import json

CACHE_MAX_AGE_H = 20  # Horas máximas de validez del cache

def _find_cache_json() -> str:
    """
    Busca scan_cache.json en múltiples rutas posibles.
    FIX: Streamlit Cloud usa CWD diferente según el despliegue.
    Buscamos en orden de probabilidad para garantizar que lo encontremos.
    """
    candidates = [
        # 1. CWD del proceso (Streamlit Cloud pone CWD = raíz del repo)
        os.path.join(os.getcwd(), "data", "scan_cache.json"),
        # 2. Relativa al archivo .py (desarrollo local)
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "scan_cache.json"),
        # 3. Ruta relativa simple (fallback)
        "data/scan_cache.json",
        # 4. Mismo directorio que el script
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "scan_cache.json"),
        # 5. CWD directo
        "scan_cache.json",
    ]
    for path in candidates:
        if os.path.exists(path):
            return path
    return candidates[0]  # Devuelve la ruta preferida aunque no exista


def load_nightly_cache() -> dict | None:
    """
    Carga los resultados del job nocturno si existen y son recientes (<20h).
    FIX v4.0.1: recalcula la ruta en cada llamada para Streamlit Cloud.
    """
    try:
        cache_path = _find_cache_json()
        if not os.path.exists(cache_path):
            return None
        with open(cache_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        generated_at_str = data.get("generated_at", "2000-01-01T00:00:00")
        generated_at = datetime.fromisoformat(generated_at_str)
        age_hours = (datetime.utcnow() - generated_at).total_seconds() / 3600
        if age_hours > CACHE_MAX_AGE_H:
            return None  # Cache obsoleto
        return data
    except Exception:
        return None


def get_nightly_cache_info() -> dict:
    """
    Devuelve metadata del cache nocturno sin cargar todo el JSON.
    """
    try:
        cache_path = _find_cache_json()
        if not os.path.exists(cache_path):
            return {"available": False, "reason": "Archivo no encontrado"}
        with open(cache_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        generated_at_str = data.get("generated_at", "2000-01-01T00:00:00")
        generated_at = datetime.fromisoformat(generated_at_str)
        age_hours = (datetime.utcnow() - generated_at).total_seconds() / 3600
        if age_hours > CACHE_MAX_AGE_H:
            return {"available": False, "reason": f"Cache obsoleto ({age_hours:.1f}h > {CACHE_MAX_AGE_H}h)"}
        return {
            "available": True,
...
def render():
    ...
    # TAB 1: SCANNER MEJORADO CON PERSISTENCIA + JOB NOCTURNO
    with tab1:

        # ── BANNER JOB NOCTURNO (FIX v4.0.1) ──────────────────────────────
        cache_info = get_nightly_cache_info()
        if cache_info["available"]:
            mkt_phase = cache_info.get("market_status", {}).get("phase", "N/A")
            mkt_score_val = cache_info.get("market_status", {}).get("score", 0)
            st.markdown(f"""
            <div style="background:linear-gradient(90deg,rgba(0,255,173,.12),rgba(0,255,173,.03));
                        border:1px solid rgba(0,255,173,.4);border-radius:10px;padding:14px 18px;
                        margin-bottom:12px;font-family:'Courier New',monospace;">
                <div style="color:#00ffad;font-size:1rem;font-weight:bold;margin-bottom:4px;">
                    ✅ JOB NOCTURNO DISPONIBLE
                </div>
                <div style="color:#ccc;font-size:.85rem;">
                    Generado hace <b>{cache_info['age_hours']}h</b> &nbsp;|&nbsp;
                    <b>{cache_info['total_candidates']}</b> candidatos &nbsp;|&nbsp;
                    S&P 500: <b>{cache_info['sp500_count']}</b> acciones &nbsp;|&nbsp;
                    Mercado: <b>{mkt_phase} ({mkt_score_val}/100)</b>
                </div>
            </div>
            """, unsafe_allow_html=True)
            if st.button("🌙 CARGAR RESULTADOS DEL JOB NOCTURNO", type="primary", use_container_width=True):
                data = load_nightly_cache()
                if data:
                    st.session_state.scan_candidates = data.get("candidates", [])
                    st.session_state.scan_timestamp = data.get("generated_at", "")[:16].replace("T", " ")
                    st.session_state.last_scan_params = {"source": "nightly_job"}
                    st.success(f"✅ Cargados {len(st.session_state.scan_candidates)} candidatos del job nocturno")
                    st.rerun()
        else:
            reason = cache_info.get("reason", "desconocido")
            st.markdown(f"""
            <div style="background:rgba(255,152,0,.08);border:1px solid rgba(255,152,0,.4);
                        border-radius:10px;padding:14px 18px;margin-bottom:12px;
                        font-family:'Courier New',monospace;">
                <div style="color:#ff9800;font-size:1rem;font-weight:bold;margin-bottom:4px;">
                    ⚠️ JOB NOCTURNO NO DISPONIBLE — {reason}
                </div>
                <div style="color:#aaa;font-size:.82rem;">
                    El scan en vivo puede tardar 5-10 minutos. El job corre automáticamente lun-vie 03:00 UTC.
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col4:
            st.markdown("<br>", unsafe_allow_html=True)
            scan_button = st.button("🔍 ESCANEAR EN VIVO", use_container_width=True)
    ...
    # TAB 3: METODOLOGÍA COMPLETA ACTUALIZADA v4.0.1
    with tab3:
        if st.session_state.scan_candidates:
            st.markdown(f"""
            <div class="saved-results-banner">
                📊 <strong>Hay {len(st.session_state.scan_candidates)} candidatos guardados</strong>
        ...
        st.markdown("""
        <style>
        .methodology-section h3 { color: #00ffad !important; margin-top: 30px;
            border-bottom: 2px solid #1a1e26; padding-bottom: 10px; }
        .methodology-section h4 { color: #ff9800 !important; margin-top: 20px; }
        </style>
        """, unsafe_allow_html=True)
        st.markdown('<div class="methodology-section">', unsafe_allow_html=True)

        # ── Sección CAN SLIM actualizada ──────────────────────────────────
        st.markdown(EDUCATIONAL_CONTENT["guia_completa"])

        # Box adicional con la tabla de scores exactos v4.0.1
        st.markdown(f"""
        <div style="background:{COLORS['bg_card']};border:1px solid rgba(0,255,173,.3);
                    border-radius:10px;padding:16px;margin:12px 0;">
            <div style="color:#00ffad;font-family:'Courier New',monospace;font-size:1rem;
                        font-weight:bold;margin-bottom:10px;">
                📊 TABLA DE PUNTUACIÓN EXACTA v4.0.1
            </div>
            <table style="width:100%;font-family:'Courier New',monospace;font-size:.82rem;
                          border-collapse:collapse;">
                <tr style="background:rgba(0,255,173,.1);color:#00ffad;">
                    <th style="padding:6px 10px;text-align:left;">Criterio</th>
                    ...
                    <th style="padding:6px;text-align:center;">D</th>
                    <th style="padding:6px;text-align:center;">Máx</th>
                </tr>
                <tr style="border-bottom:1px solid #2a2e36;color:#ccc;">
                    <td style="padding:5px 10px;">C — EPS Growth</td>
                    <td style="text-align:center;color:#00ffad;">&gt;50%=20 / &gt;25%=15</td>
                    <td style="text-align:center;color:#ff9800;">15-25%=10</td>
                    <td style="text-align:center;color:#f44;">0-15%=5</td>
                    <td style="text-align:center;color:#888;">&lt;0%=0</td>
                    <td style="text-align:center;font-weight:bold;">20</td>
                </tr>
                <tr style="border-bottom:1px solid #2a2e36;color:#ccc;">
                    <td style="padding:5px 10px;">A — Annual EPS</td>
                    <td style="text-align:center;color:#00ffad;">&gt;50%=15 / &gt;25%=12</td>
                    <td style="text-align:center;color:#ff9800;">15-25%=8</td>
                    <td style="text-align:center;color:#f44;">0-15%=4</td>
                    <td style="text-align:center;color:#888;">&lt;0%=0</td>
                    <td style="text-align:center;font-weight:bold;">15</td>
                </tr>
                <tr style="border-bottom:1px solid #2a2e36;color:#ccc;">
                    <td style="padding:5px 10px;">N — Near High 52w</td>
                    <td style="text-align:center;color:#00ffad;">&lt;3%=15 / &lt;10%=12</td>
                    <td style="text-align:center;color:#ff9800;">&lt;20%=8</td>
                    <td style="text-align:center;color:#f44;">&lt;30%=4</td>
                    <td style="text-align:center;color:#888;">&gt;30%=0</td>
                    <td style="text-align:center;font-weight:bold;">15</td>
                </tr>
                <tr style="border-bottom:1px solid #2a2e36;color:#ccc;">
                    <td style="padding:5px 10px;">S — Volume Ratio</td>
                    <td style="text-align:center;color:#00ffad;">&gt;2x=10 / &gt;1.5x=8</td>
                    <td style="text-align:center;color:#ff9800;">&gt;1x=5</td>
                    <td style="text-align:center;color:#f44;">&lt;1x=2</td>
                    <td style="text-align:center;color:#888;">—</td>
                    <td style="text-align:center;font-weight:bold;">10</td>
                </tr>
                <tr style="border-bottom:1px solid #2a2e36;color:#ccc;">
                    <td style="padding:5px 10px;">L — RS Rating</td>
                    <td style="text-align:center;color:#00ffad;">&gt;90=15 / &gt;80=12</td>
                    <td style="text-align:center;color:#ff9800;">&gt;70=8</td>
                    <td style="text-align:center;color:#f44;">&gt;60=4</td>
                    <td style="text-align:center;color:#888;">&lt;60=0</td>
                    <td style="text-align:center;font-weight:bold;">15</td>
                </tr>
                <tr style="border-bottom:1px solid #2a2e36;color:#ccc;">
                    <td style="padding:5px 10px;">I — Institutional</td>
                    <td style="text-align:center;color:#00ffad;">&gt;80%=10 / &gt;60%=8</td>
                    <td style="text-align:center;color:#ff9800;">&gt;40%=5</td>
                    <td style="text-align:center;color:#f44;">&gt;20%=3</td>
                    <td style="text-align:center;color:#888;">&lt;20%=0</td>
                    <td style="text-align:center;font-weight:bold;">10</td>
                </tr>
                <tr style="color:#ccc;">
                    <td style="padding:5px 10px;">M — Market Score</td>
                    <td style="text-align:center;color:#00ffad;">&gt;=80=15</td>
                    <td style="text-align:center;color:#ff9800;">&gt;=60=10</td>
                    <td style="text-align:center;color:#f44;">&gt;=40=5</td>
                    <td style="text-align:center;color:#888;">&lt;40=0</td>
                    <td style="text-align:center;font-weight:bold;">15</td>
                </tr>
            </table>
            <div style="color:#888;font-size:.78rem;margin-top:8px;font-family:'Courier New',monospace;">
                RS Rating = percentil real calculado vs universo S&P 500 completo (ponderación 40/20/20/20 trimestral IBD)
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Nota explicativa sobre los IBD Ratings calculados vs oficiales
        st.info(
            "⚠️ **Nota:** Los IBD Ratings mostrados son **calculados internamente** con metodología "
            "aproximada a partir de datos de Yahoo Finance. No son los ratings oficiales de IBD "
            "(investors.com). El RS Rating SÍ es un percentil real calculado comparando cada acción "
            "contra todas las demás del universo con la ponderación 40/20/20/20 de IBD."
        )

        st.markdown(EDUCATIONAL_CONTENT["trend_template_minervini"])

        # Box arquitectura técnica v4.0.1
        st.markdown(f"""
        <div style="background:{COLORS['bg_card']};border:1px solid rgba(0,200,255,.3);
                    border-radius:10px;padding:16px;margin:16px 0;">
            <div style="color:#00d9ff;font-family:'Courier New',monospace;font-size:1rem;
                        font-weight:bold;margin-bottom:10px;">
                ⚙️ ARQUITECTURA v4.0.1 — JOB NOCTURNO + CACHE JSON
            </div>
            <div style="font-family:'Courier New',monospace;font-size:.82rem;color:#ccc;line-height:1.7;">
                <b style="color:#00ffad;">Capa 1 — GitHub Actions (03:00 UTC lun-vie):</b><br>
                &nbsp;&nbsp;nightly_scan.py → descarga batch S&P 500 → scores CAN SLIM → data/scan_cache.json → git push<br><br>
                <b style="color:#00ffad;">Capa 2 — Streamlit Cloud (lectura instantánea):</b><br>
                &nbsp;&nbsp;Al abrir la app → busca scan_cache.json → si válido (&lt;20h) → botón verde → carga en &lt;1s<br><br>
                <b style="color:#ff9800;">Ventajas:</b> 0 rate limits en producción · 100 usuarios leen el mismo JSON · scan en vivo como fallback<br><br>
                <b style="color:#ff9800;">FIX v4.0.1:</b> _to_float() evita "Series ambiguous" · get_spy() normaliza MultiIndex · 11 tickers delistados eliminados
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # TAB 4: ML PREDICTIVO — ESTADO HONESTO Y HOJA DE RUTA
    with tab4:
        if st.session_state.scan_candidates:
            st.markdown(f"""
            <div class="saved-results-banner">
        ...
        st.header("🤖 ML Predictivo — Estado y Hoja de Ruta")

        # Estado actual honesto
        st.markdown(f"""
        <div style="background:rgba(255,152,0,.08);border:1px solid rgba(255,152,0,.4);
                    border-radius:10px;padding:16px;margin:10px 0;">
            <div style="color:#ff9800;font-family:'Courier New',monospace;font-size:1rem;
                        font-weight:bold;margin-bottom:8px;">
                📊 ESTADO ACTUAL: EN ENTRENAMIENTO
            </div>
            <div style="font-family:'Courier New',monospace;font-size:.85rem;color:#ccc;line-height:1.6;">
                El modelo actualmente devuelve ~50% para todas las acciones porque <b>aún no tiene datos 
                de entrenamiento históricos</b>. La probabilidad que ves en el scanner es un placeholder.<br><br>
                <b style="color:#ff9800;">Para que el ML sea útil se necesitan:</b><br>
                &nbsp;• 6-12 meses de datos: para cada scan, guardar métricas + retorno real posterior 3 meses<br>
                &nbsp;• ~200+ samples mínimo para entrenamiento válido (idealmente 500+)<br>
                &nbsp;• Target binario: ¿superó la acción al SPY en los 3 meses siguientes? (1/0)
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            st.subheader("📅 Hoja de Ruta ML")
            st.markdown("""
            | Mes | Hito |
            |-----|------|
            | Ahora | Cada scan nocturno guarda métricas en histórico |
            | Mes 3 | Calcular retorno real de candidatos del mes 1 |
            | Mes 4-6 | ~200 samples → primer entrenamiento GradientBoosting |
            | Mes 6-12 | ~500 samples → validación temporal real |
            | Mes 6+ | Modelo en producción con probabilidades reales |
            """)
            st.info("💡 El modelo ya está implementado (GradientBoostingClassifier). Solo falta alimentarlo con datos reales.")

        with col2:
            st.subheader("⚙️ Features del Modelo")
            if not SKLEARN_AVAILABLE:
                st.warning("scikit-learn no instalado. Añade `scikit-learn` a requirements.txt")
            else:
                ml = CANSlimMLPredictor()
                try:
                    ...
                    st.markdown("""
                    **Features planificadas (9 métricas CAN SLIM):**
                    - earnings_growth, eps_growth, revenue_growth
                    - pct_from_high, volume_ratio, rs_rating
                    - inst_ownership, market_score, price_momentum
                    """)

        st.subheader("🔮 Predicción Individual (Placeholder)")
        st.warning("⚠️ Las probabilidades actuales no son predictivas. Son valores de demostración hasta que el modelo sea entrenado.")
        pred_ticker = st.text_input("Ticker", "NVDA").upper()
        if st.button("Calcular Probabilidad (Demo)", disabled=not SKLEARN_AVAILABLE):
            try:
                ...
                if result:
                    ...
                    <div style="background:{COLORS['bg_card']};padding:20px;border-radius:10px;text-align:center;">
                        <h3>Probabilidad de Outperformance (Demo)</h3>
                        <h1 style="color:{color};font-size:4rem;margin:10px 0;">{prob:.1%}</h1>
                        <p style="color:#888;">⚠️ Valor placeholder — no usar para decisiones de inversión</p>
                    </div>
                    """, unsafe_allow_html=True)
            except Exception as e:
                st.error(f"Error: {str(e)}")

    # TAB 5: BACKTESTING — ESTADO HONESTO Y RECOMENDACIÓN
    with tab5:
        if st.session_state.scan_candidates:
            st.markdown(f"""
            <div class="saved-results-banner">
        ...
        st.header("📈 Backtesting — Estado y Recomendación")

        # Estado actual honesto
        st.markdown(f"""
        <div style="background:rgba(242,54,69,.08);border:1px solid rgba(242,54,69,.4);
                    border-radius:10px;padding:16px;margin:10px 0;">
            <div style="color:#f23645;font-family:'Courier New',monospace;font-size:1rem;
                        font-weight:bold;margin-bottom:8px;">
                ⚠️ ADVERTENCIA: LOS NÚMEROS ACTUALES SON PLACEHOLDER
            </div>
            <div style="font-family:'Courier New',monospace;font-size:.85rem;color:#ccc;line-height:1.6;">
                Los resultados que aparecen (+145.3%, Sharpe 1.85, etc.) son <b>valores hardcodeados de demostración</b>
                que NO representan rendimiento real de la estrategia.<br><br>
                Zipline (la librería de backtesting seleccionada inicialmente) es <b>excesivamente compleja</b>
                para este caso de uso y difícil de instalar en Streamlit Cloud.<br><br>
                <b style="color:#ff9800;">Recomendación: implementar con pandas (2-3 días de trabajo).</b>
            </div>
        </div>
        """, unsafe_allow_html=True)

        st.subheader("🛠️ Plan de Implementación Recomendado (pandas)")

        col1, col2 = st.columns(2)
        with col1:
            st.markdown("""
            **Estrategia de backtest propuesta:**
            - **Entrada:** Score > 80 AND Composite > 90 AND Stage 2 (8/8) AND Market Score > 70
            - **Stop loss:** -7% desde entrada
            - **Profit target:** +20-25%
            - **Posición:** equal-weight (todos al mismo peso)
            - **Universo:** S&P 500, 5 años histórico
            - **Benchmark:** Buy&Hold SPY mismo período
            """)
        ...
        with col2:
            ...
            **Comparativa de opciones:**
            ...
            | Opción | Complejidad | Tiempo | En Streamlit Cloud |
            |--------|-------------|--------|--------------------|
            | **pandas** (recomendado) | Baja | 2-3 días | ✅ Sí |
            | vectorbt | Media | 1 semana | ✅ Sí |
            | backtrader | Media | 1 semana | ✅ Sí |
            | zipline (actual) | Alta | 1-2 meses | ❌ Difícil |
            """)

        st.subheader("▶️ Simulación Demo (Resultados No Reales)")
        st.warning("Los resultados a continuación son PLACEHOLDER. No usar para decisiones.")
        ...
        with st.expander("Parámetros de Estrategia"):
        ...
        if st.button("▶️ Ejecutar Demo (Valores No Reales)", type="secondary"):
            st.warning("⚠️ Recuerda: estos números son placeholders de demostración")
            metrics_col1, metrics_col2, metrics_col3, metrics_col4 = st.columns(4)
            metrics_col1.metric("Total Return", "+145.3%", "+45.2% vs SPY", delta_color="normal")
